...
public class ParingsActivity extends Activity {

	private TextView mCounterTextView;
	private TextView mRoundTextView;
	private ListView mParingListView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mCounterTextView = (TextView) findViewById(R.id.paring_counterTextView);
		mRoundTextView = (TextView) findViewById(R.id.paring_roundTextView);
		mParingListView = (ListView) findViewById(R.id.paring_paringListView);
		mStartButton = (Button) findViewById(R.id.paring_startCounterButton);
		mEndRoundButton = (Button) findViewById(R.id.paring_endRoundButon);
	...
	private void init (){
		...
		// start with setting it on adapters
		/*Bundle b = getIntent().getExtras();
		if (b != null && b.containsKey(BundleConst.BUNDLE_KEY_NAMES_LIST)
				&& b.containsKey(BundleConst.BUNDLE_KEY_ROUNDS_INT)) {
			// set for 1rst round
			AlgorithmFactory.configure(new ParingAlgorithm(b
					.getStringArrayList(BundleConst.BUNDLE_KEY_NAMES_LIST), b
					.getInt(BundleConst.BUNDLE_KEY_ROUNDS_INT)));
		...
		if (mGameList == null || mGameList.isEmpty()) {
			...
			mCustomDialogFragment.show(getFragmentManager(), TAG_DIALOG);
		}
